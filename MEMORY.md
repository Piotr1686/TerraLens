...
### [2026-06-15] Warstwa heatmap KOMPLETNA (CVA realne) + plan ESRGAN-w-PMTiles

- **CVA realne heatmapy (DONE, commit `f46186f`):** domknięcie całej warstwy heatmap — SSIM+CVA+NDVI to teraz realne mapy z pipeline'u, demo GIBS usunięte. `build_region_cva_heatmaps()` w `scripts/build_heatmaps.py` liczy ΔE w LAB (`processors/cva.compute_cva`) z tych samych tile'ów HLS_RGB co SSIM (z=7), z identycznym cloud-maskingiem (`apply_cloud_mask`, chmura→NaN, inaczej pojawienie się chmury = fałszywa zmiana koloru); ΔE<10 (próg percepcji CIE)→przezroczyste; colormap `plasma`. Deploy `deploy-heatmaps` → HF `{region}_cva_heatmap/{z}/{x}/{y}.png`. Render: amazonia 49, dubai 1, arctic 81 tile. Front: `cva` w `REAL_METRICS` (z=7), usunięty martwy fallback demo GIBS (`TileLayer`, `Bands721`). Zweryfikowane w bundlu prod: `cva_heatmap=1`, `Bands721=0`. Wzorzec identyczny jak [2026-06-04] SSIM.
- **Obserwacja deploy:** auto-promocja Vercela zadziałała w pełni — nowy deploy sam przejął wszystkie aliasy (w tym `terra-lens-zeta`); `vercel promote` zwrócił 409 „already current production". Historyczny problem z ręcznym `alias set` (patrz [[bug_vercel_stale_deploy_gitignore]]) już nie blokuje. `NODE_EXTRA_CA_CERTS` = `%TEMP%\win-ca-bundle.pem` (regenerowalny z `Cert:\LocalMachine\Root`).

### [2026-06-16] ESRGAN-w-PMTiles WDROŻONY (pilot amazonia) + pivot na Sentinel-2 (S10)

- **ESRGAN-w-PMTiles DONE (commit `1088c52`):** `process` (+`--date`, filtr snapshotu) i `export` (+`--sr`, źródło `tiles/{layer}_SR/`) w `__main__.py`. Render 52 kafle 2048² SR (amazonia z6+z7, data 2023-07-01), export `amazonia_v20260616_190200.pmtiles` (14 MB), deploy HF. Zweryfikowane e2e: kafel z7 z HF = 2048² SR (nie raw 512²). Front bez zmian (Architektura A, manifest-driven). To **domyka lukę AI/ML** dla amazonii.
- **Najczystsza data = już użyta:** skan `scripts/scan_cloud_cover.py` (reuse `cloud_proxy_qa`+`cloud_fraction`) wykazał, że 2023-07-01 to OBIEKTYWNIE najczystsza data amazonii (cloud_frac 0.219; reszta 0.29–0.85, tropik). Re-render innej daty bezcelowy.
- **Sufit Architektury A:** `pickGibsZoom` (`usePMTilesLayer.ts:34`) przy zoom≥7.5 prosi o z8 (brak w PMTiles → fallback rozciąga z7). SR 4× ≈120 m/px — NIE street-level. To granica GIBS HLS, nie bug. „Brak różnicy" na prod = lokalny cache przeglądarki (produkcyjna ścieżka HF→pmtiles zweryfikowana jako poprawny SR).
- **PIVOT zatwierdzony → Sprint S10 Explore Mode na Sentinel-2 (10 m, dowolny obszar):** źródło **Microsoft Planetary Computer** (hostowany tiler `pctiler`/titiler-pgstac — reprojekcja UTM→WebMercator i RGB po stronie MPC, ZERO własnego GDAL). **PoC udany** (`scripts/poc_sentinel2.py`): STAC search `sentinel-2-l2a` → scena 0.0% chmur → kafel `/api/data/v1/item/tiles/WebMercatorQuad/{z}/{x}/{y}@1x.png?collection=sentinel-2-l2a&item=<id>&assets=visual` (asset `visual` = gotowy TCI true-color, BEZ `color_formula`); ostrość 10 m potwierdzona wizualnie (Dubai street-level). Front reużyje deck.gl `TileLayer` (kamera ma już `controller` = free-zoom). **Backend: ZERO-BACKEND (decyzja)** — Cloudflare odpadł (brak konta), ale okazał się zbędny: tiler pctiler jest PUBLICZNY (kafel pobrany bez tokenu — `pc.sign` tylko dla surowych COG, nie tilera), a STAC + data-API zwracają `Access-Control-Allow-Origin: *` → front woła MPC BEZPOŚREDNIO (STAC scene-pick client-side + `TileLayer`→pctiler URL). Escape hatch przy rate-limicie: cache-shim jako Vercel Function (front już na Vercelu).
- **SSL niuans (rozszerza [[feedback_windows_ssl]]):** `pystac_client` HARDKODUJE `verify=True` (`stac_api_io.py:212`) → projektowy wzorzec `verify=False` NIE działa. Fix: env `REQUESTS_CA_BUNDLE` + `SSL_CERT_FILE` = `%TEMP%\win-ca-bundle.pem` (realna weryfikacja Windows-roots, nie wyłączanie).
- **ESRGAN-w-PMTiles (ZAPLANOWANE, nie wykonane):** ostatni element luki AI/ML. Kafle HLS_RGB na globie to wciąż surowy GIBS — `export` (`__main__.py:249`) ma na sztywno `tile_dir = tiles/{layer}` i nigdy nie tyka `tiles/{layer}_SR/` produkowanego przez `process` (Real-ESRGAN 4×). Render SR praktycznie niezrobiony (raw z7=135 kafli, `_SR`=1 próbka PoC). **Decyzja — Architektura A:** trzymać 2048² SR w tych samych slotach (z,x,y) z6/z7; front BEZ ZMIAN (URL z manifestu `App.tsx:84-89`, per-tile `BitmapLayer` używa pełnej tekstury). Wariant B (re-tiling do z8/z9) odłożony. Kod: tylko `__main__.py` — `process` (+`--date`, by SR-ować snapshot a nie 19 dat), `export` (+`--sr`). Pilot amazonia. Plan: `~/.claude/plans/serialized-plotting-twilight.md`.

### [2026-06-17] S10 Explore Mode FRONTEND wdrożony — kaskada NAIP→S2 + oś czasu (zero-backend)

- **Tryb Explore działa (commity `ccea853`→`2d05d6f`):** wyszukiwarka miejsca (Nominatim, `lib/geocode.ts`) → front woła MPC bezpośrednio (potwierdzone CORS `*`, zero-backend). `lib/mpc.ts` zrefaktorowane na **resolver list scen per źródło** `listScenes()` (NAIP limit 24 + S2 limit 250, sortby datetime desc, BEZ filtra chmur). Hooki: `useExploreScenes` (pobiera listy, race-guard `reqId` + AbortController), `useExploreLayer` (buduje `TileLayer` z wybranej sceny, mirror `Globe.makeTileLayer`, `extent=scene.bbox`), `useExploreSelection` (stan: źródło/filtr/index). UI `ExploreControls.tsx`.
- **Kaskada źródeł NAIP→S2 (decyzja: realne>syntetyczne):** NAIP (~0.6 m, USA, kolekcja `naip`, render `assets=image` `asset_bidx=image|1,2,3`, z18) gdy dostępne, inaczej Sentinel-2 (10 m globalnie, `assets=visual`, z14). Kafle **`@2x`** (retina 512 px). Cap kamery per źródło = `scene.maxZoom + 0.5` (GlobeView NIE ma opcji `maxZoom` na kontrolerze → clamp ręczny w `Globe.onViewStateChange`). ESRGAN-dopał **świadomie odłożony** (real sub-metr bije syntetyczny SR, ryzyko artefaktów, zero-backend; warstwa AI/ML już pokazana w pre-bake). Szczegóły: [[project_explore_source_cascade]].
- **Oś czasu + ręczny przełącznik źródła:** suwak dat (lewo=stare/prawo=nowe) z etykietą data+%chmur; przełącznik NAIP↔S2 (NAIP disabled poza USA); toggle „Clear skies only" (<20%, OFF domyślnie — pochmurne daty widoczne na życzenie usera). Każda klatka = realna scena w natywnej rozdzielczości (jakość zachowana).
- **Odporność na awarie MPC:** `stacSearch` ma timeout per próba (`AbortSignal.timeout`+`.any`) + retry na 504/503; NAIP failure NIE blokuje S2; komunikat błędu łagodny. Tła: search endpoint MPC bywa przejściowo 504 (~30 s) niezależnie od kształtu zapytania (health `GET /stac/v1/` zostaje 200) — to degradacja po stronie Microsoftu, nie nasz bug.
- **Znane ograniczenie (T10.5):** oś czasu S2 sięga wstecz tylko ~2025 (`limit:250` ≈ <2 lata przy ~2 orbitach/punkt) — do naprawy paginacją STAC lub best-per-month. Renderer 3D bez zmian (deck.gl GlobeView, [[feedback_deckgl_gibs_tiles]]).
