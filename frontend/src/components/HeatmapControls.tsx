...
const METRICS: { id: HeatmapMetric; label: string; desc: string; color: string }[] = [
  { id: 'ssim', label: 'SSIM', desc: 'structural changes — terrain & buildings', color: 'bg-blue-500/70 ring-blue-400' },
  { id: 'ndvi', label: 'NDVI', desc: 'vegetation index — forests & crops',       color: 'bg-green-500/70 ring-green-400' },
  { id: 'cva',  label: 'CVA',  desc: 'colour change — fires, droughts',          color: 'bg-orange-500/70 ring-orange-400' },
]
...
export function HeatmapControls({ metric, opacity, onMetricChange, onOpacityChange }: Props) {
  return (
    <div className="absolute right-4 top-4 flex flex-col gap-3 rounded-xl bg-black/50 p-4 backdrop-blur w-44">
      <p className="text-xs font-semibold uppercase tracking-wider text-white/50">Change Overlay</p>
